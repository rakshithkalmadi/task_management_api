...
def create_new_log(log: LogCreate, current_user: User = Depends(get_current_active_user)):
...
def update_existing_log(log_id: str, log: LogUpdate, current_user: User = Depends(get_current_active_user)):
    db_log = get_log(log_id)
    if db_log is None:
        raise HTTPException(status_code=404, detail="Log not found")
...
def delete_existing_log(log_id: str, current_user: User = Depends(get_current_active_user)):
    db_log = get_log(log_id)
    if db_log is None:
        raise HTTPException(status_code=404, detail="Log not found")
    delete_log(log_id)
